...
from transcriptforge_api.workers.assay_experiment import run_assay_experiment_workflow
from transcriptforge_api.workers.assay_study import run_assay_study_workflow
...
@celery_app.task(name="transcriptforge.experiments.run")  # type: ignore[untyped-decorator]
def run_assay_experiment(run_id: str) -> dict[str, Any]:
    """Execute one already-frozen pre-lock Development Experiment."""
    return run_assay_experiment_workflow(run_id)


@celery_app.task(name="transcriptforge.studies.run")  # type: ignore[untyped-decorator]
def run_assay_study(run_id: str) -> dict[str, Any]:
    """Execute one frozen post-lock Analytical Study."""
    return run_assay_study_workflow(run_id)
